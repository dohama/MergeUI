...
> 작성: 에이전트 A (시니어 PM) | 최초 2026-04-12 | 최종 업데이트 2026-04-15
> 목적: 아이디어에서 실제 수익 발생까지의 전체 액션 플랜
> 전체 프로젝트 히스토리: `docs/plans/project-history.md` 참조

---

## 현재 진행 상황 요약 (2026-04-15 기준)

### 프론트엔드 — 100% 완료
- **총 39개 페이지 전체 완성** (blog 제외, 캡틴 결정)
  - 공개 17개 | 인증 5개 | 결제 2개 | 구독자 5개 | 관리자 9개
- 레거시 파일 4개 리다이렉트 처리 완료

### 인프라 개선 (2026-04-15)
- **CSS 외부 분리 완료**: tokens.css, reset.css, nav.css, sidebar.css
- **`--merge-*` 토큰 통일**: 라이트/다크 모드 토큰 + 하위호환 별칭
- **auth.js 공통 모듈**: 인증 체크 + Nav 상태 전환 + 테마 토글 (try-catch 포함)
- **GA4 analytics.js**: 28개 페이지에 트래킹 코드 삽입 + 커스텀 이벤트 헬퍼
- **SEO 메타/OG 태그**: 12개 누락 페이지에 추가 (description + OG + canonical + twitter)

### 6명 합동 검증 결과 (2026-04-15)
- Critical 6건 (보안 관련 → 백엔드 연동 시 해결)
- Major 12건 (CSS 분리로 6건 해결, 나머지 백엔드 연동 시)
- 상세: 합동 검증 리포트 참조
...
- 1600px 와이드 레이아웃
- 수익화 최우선, 실 서비스 오픈 목표
- blog 제외 (2026-04-14 캡틴 결정)
- 보안 이슈는 백엔드 개발 시 일괄 처리 (2026-04-15 캡틴 결정)
- Tailwind 전환 불필요, 순수 CSS 변수 체계 유지 (2026-04-15 캡틴 결정)
- 판매 템플릿은 1안(MVP) HTML/CSS 원본 + 프레임워크별 가이드 (2026-04-15 캡틴 결정)
- 접속자 관리: 1안(MVP) GA4만 우선 적용 → 런칭 후 결제/구독자 데이터 쌓이면 2안(GA4 + Supabase 이벤트 로깅) 확장. 자체 정성분석(Hotjar 스타일)은 구독자 100명+ 이후 (2026-04-16 캡틴 결정)

### 다음 우선순위
1. **백엔드 구축** — 인증(Supabase Auth) + 결제(Lemonsqueezy) + 다운로드 API
2. 프론트-백엔드 연동 (하드코딩 데이터 → API 호출)
3. 보안 강화 (bcrypt, JWT role 체크, CSRF, XSS sanitize)
4. E2E 테스트 + 런칭 전 최종 QA
